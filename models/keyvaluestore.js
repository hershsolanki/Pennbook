...
(function() {
   ...
   * @param search
   *
   * Callback returns a list of objects with keys "key" and "value"
   */
  keyvaluestore.prototype.getPrefixKeys = function(search, callback) {
    var self = this;
    if (self.inx === -1){
      callback("Error using table - call init first!", null)
      return
    ...
    db.query(params, function(err, data) {
      if (err || data.Items.length == 0)
        ...
        var items = [];
        for (var i = 0; i < data.Items.length; i++) {
          items.push({"key": data.Items[i].key.S, "value": data.Items[i].value.S});
        }
        callback(err, items);
      }
    });
  ...
  /**
   * Replace value of entry matching the keyword and inx
   * Assumes that the existing value is a JSON string! 
   *
   * @param keyword
   * @param inx
   * @param value - value to replace the value of the original item
   */
  keyvaluestore.prototype.replace = function(keyword, inx, value, callback) {
    var self = this;
    if (self.inx === -1){
      callback("Error using table - call init first!", null)
      return
